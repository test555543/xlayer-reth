...
async fn bundle_range_limits(rbuilder: LocalInstance) -> eyre::Result<()> {
    ...
    assert!(send_bundle(
        &driver,
        BundleOpts::default().with_block_number_max(next_valid_block + MAX_BLOCK_RANGE_BLOCKS + 1)
    )
    .await
    .is_err());
    ...
    .await
    .is_ok());
    assert!(send_bundle(&driver, BundleOpts::default().with_block_number_max(next_valid_block))
        .await
        .is_ok());
    ...
    assert!(send_bundle(&driver, BundleOpts::default().with_block_number_min(current_block))
        .await
        .is_ok());
    assert!(send_bundle(&driver, BundleOpts::default().with_block_number_min(default_max - 1))
        .await
        .is_ok());
    assert!(send_bundle(&driver, BundleOpts::default().with_block_number_min(default_max))
        .await
        .is_ok());
